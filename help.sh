...
echo ""
echo "Istio Installation Options:"
echo "  Default installation: istioctl install --set profile=default -y"
echo ""
echo "Istio Addons Installation Options:"
echo "  Install all addons at once: ./install-addons.sh"
echo "  Install Grafana only: kubectl apply -f istio-grafana.yaml"
echo "  Install all addons with one command: kubectl apply -f istio-full.yaml"
echo ""
...
echo ""
echo "6. install-addons.sh"
echo "   Purpose: Install all Istio observability addons at once"
echo "   Usage: ./install-addons.sh"
echo "   Description:"
echo "     - Installs Grafana, Kiali, Jaeger, and Prometheus"
echo "     - Provides a complete observability stack for the service mesh"
echo "     - Must be run after Istio is installed but before accessing dashboards"
echo ""
echo "7. fault-injection.sh"
...
echo ""
echo "9. port-forward.sh"
echo "   Purpose: Set up port-forwarding for macOS users with connectivity issues"
echo "   Usage: ./port-forward.sh"
echo "   Description:"
echo "     - Creates a port-forward from localhost:8080 to the Istio Ingress Gateway"
echo "     - Makes services accessible via http://localhost:8080/app1 and http://localhost:8080/app2"
echo "     - Useful for macOS users who have trouble accessing the Minikube cluster IP directly"
echo "     - Keeps running until Ctrl+C is pressed"
echo ""
echo "10. test-connectivity.sh"
echo "   Purpose: Test connectivity to the deployed services"
echo "   Usage: ./test-connectivity.sh"
echo "   Description:"
echo "     - Tests pod-to-pod connectivity (service discovery)"
echo "     - Tests direct cluster IP connectivity"
echo "     - Tests localhost connectivity via port-forwarding"
echo "     - Provides recommendations based on the test results"
echo "     - Helps diagnose connectivity issues, especially on macOS"
echo ""
echo "11. diagrams.md"
